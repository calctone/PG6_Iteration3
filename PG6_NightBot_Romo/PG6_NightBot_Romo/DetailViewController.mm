...
- (void)sendTwilioText: (NSString*)message {
    ...
    NSString *kToNumber = @"+19137306622"; //TODO PHONE NUMBER GOES HERE!
...
- (void)socket:(GCDAsyncSocket *)sender didAcceptNewSocket:(GCDAsyncSocket *)newSocket
{
    ...
    dispatch_once(&onceToken, ^{
        UIImage *sendImage = [UIImage imageNamed:@"cane.jpg"];//@"door_opened.jpg"];
        cv::Mat networkImage = [sendImage CVGrayscaleMat];
        NSLog(@"Channels : %d", networkImage.channels());
        
        NSData *data = UIImageJPEGRepresentation(sendImage, 0.8);
